...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  # calculate padding
  # H_pad(include both sides) = (Stride - 1) * H - Stride + H_kernel
  H_pad1 = (stride - 1) * H - stride + kernel_size_1
  W_pad1 = (stride - 1) * W - stride + kernel_size_1

  # output size of conv1
  # HH1 = (H + H_pad * 2 - H_kernel) // stride + 1
  HH1 = (H - kernel_size_1 + H_pad1) // stride + 1
  WW1 = (W - kernel_size_1 + W_pad1) // stride + 1
  ...
  # calculate padding
  # H_pad(include both sides) = (Stride - 1) * H - Stride + H_kernel
  H_pad2 = (1 - 1) * HH1 - 1 + kernel_size_2
  W_pad2 = (1 - 1) * WW1 - 1 + kernel_size_2

  # output size of conv2
  # HH1 = (H + H_pad * 2 - H_kernel) // stride + 1
  HH2 = (HH1 - kernel_size_2 + H_pad2) // stride + 1
  WW2 = (WW1 - kernel_size_2 + W_pad2) // stride + 1  

  fc_w = nn.init.kaiming_normal_(torch.empty(num_classes, channel_2 * HH2 * WW2, dtype=dtype, device=device))
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.net = nn.Sequential(OrderedDict([
      ...
      ('ReLu1', nn.ReLU()),
      ('Conv1', nn.Conv2d(Cin, Cout, kernel_size=3, stride=(2 if downsample else 1), padding=1)),
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    blocks.extend([ResNetStage(*stage, block) for stage in stage_args])
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    bottleneckOut = Cout // 4

    self.block = nn.Sequential(OrderedDict([
      ('BatchNorm1', nn.BatchNorm2d(Cin)),
      ('ReLu1', nn.ReLU()),
      ('Conv1', nn.Conv2d(Cin, bottleneckOut, kernel_size=1, stride=(2 if downsample else 1), padding=0)),
      ('BatchNorm2', nn.BatchNorm2d(bottleneckOut)),
      ('ReLu2', nn.ReLU()),
      ('Conv2', nn.Conv2d(bottleneckOut, bottleneckOut, kernel_size=3, stride=1, padding=1)),
      ('BatchNorm3', nn.BatchNorm2d(bottleneckOut)),
      ('ReLu3', nn.ReLU()),
      ('Conv3', nn.Conv2d(bottleneckOut, Cout, kernel_size=1, stride=1, padding=0))
    ]))

...
def rnn_step_backward(dnext_h, cache):
    ...
    dx = dtanh.mm(Wx.transpose(0, 1))
    dprev_h = dtanh.mm(Wh.transpose(0, 1))
    dWx = x.transpose(0, 1).mm(dtanh)
    dWh = prev_h.transpose(0, 1).mm(dtanh)
    db = dtanh.sum(dim=0)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    # Cache : List of (x, prev_h, Wx, Wh, b, next_h)
    N, T, H = dh.shape
    D = cache[0][0].shape[1]

    dx = torch.empty((N, T, D), dtype=cache[0][0].dtype, device=cache[0][0].device)
    dh0 = None
    dWx = torch.zeros_like(cache[0][2])
    dWh = torch.zeros_like(cache[0][3])
    db = torch.zeros_like(cache[0][4])

    _dprev_h = torch.zeros((N, H), dtype=cache[0][1].dtype, device=cache[0][1].device)
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    # Replace "pass" statement with your code
    N, T, V = x.shape
    reshape_x = x.reshape(-1, V)
    reshape_y = y.reshape(-1)
    loss = F.cross_entropy(reshape_x, reshape_y, ignore_index=ignore_index, reduction='sum')
    loss = loss / N
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
        ...
        self.feature_extractor = FeatureExtractor(pooling=True, dtype=dtype, device=device)
        self.feature_transform_fc = nn.Linear(input_dim, hidden_dim, dtype=dtype, device=device)
        ...
        self.output_transform_fc = nn.Linear(hidden_dim, vocab_size, dtype=dtype, device=device)
    ...
    def forward(self, images, captions):
        ...
        h0 = self.feature_transform_fc(features)
        wordvectors = self.word_embedding.forward(captions_in)
        hT = self.rnn(wordvectors, h0)
        score = self.output_transform_fc(hT)
    ...
    def sample(self, images, max_length=15):
        ...
        # Replace "pass" statement with your code
        features = self.feature_extractor.extract_mobilenet_feature(images)
        h = self.feature_transform_fc(features)
        ...
        for i in range(max_length):
          ...
          scores = self.output_transform_fc(h)
